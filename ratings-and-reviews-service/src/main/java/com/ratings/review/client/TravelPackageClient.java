 ...
 */
@FeignClient(name = "travel-package-management", url = "${package-service.url}") // Connects to the travel package service using a configurable URL
public interface TravelPackageClient {
     ...
     */
    @GetMapping("/api/packages/{id}/agent") // Maps to the endpoint that returns the agent ID for a package
    Long getAgentIdByPackage(@PathVariable("id") Long id);
}
